...
function scrambleTiles() {
   for (let i = 0; i <= allTiles.length; i++) {
      ...
      //Swap order of the insertBefore and add Before to the insert
   }
}
   ...
   for (let i = 0; i < allTiles.length; i++) {
      ...
      allTiles[i].onclick = function() {
         ...
         if (this.lastElementChild.className === "back") {
            //add another === so its comparing
            //lowercase t for this
            ...
            //add another === so its comparing
               ...
               timeID = window.setTimeout(flipBack, 1000);//counted in milliseconds. added zeros
            }
         }
      }
   }  
